...
% DATA = erddap_read(URL,[options])
% Load data from an ERDDAP Data Access Form query for filetype 'mat'
%      created with the "Just generate the URL" button. The data are 
%      returned in the output structure named DATA. Any time coordinate 
%      data is converted to Matlab datenum and datetime. This function 
...
%        weboptions('CertificateFilename','') if the SSL cert is problems
%        See help weboptions
%      Any other argument will be interpreted as a request to encode the
%        URL into safe ascii characters, which some ERDDAP servers are 
%        fussy about. This function will automatically retry with URL 
%        encoding if it strikes a server error.  
%
% John Wilkin - August 2018
%
% Copyright (c) 2026 - John L. Wilkin - jwilkin@rutgers.edu
...
try
	...
	warning("ERDDAP read failed"+newline+ ...
