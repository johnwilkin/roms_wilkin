function [thedata,thegrid,thehan] = roms_nzview(file,var,ntime,z,grd,nvec_d,uscale,varargin)
% [thedata,thegrid,thehan] = roms_nzview(file,var,time,k,grd,nvec_d,uscale,varargin)
%
% Overlay plots from a set of ROMS outputs - all for the same time index,
...
for i=1:nf
  ...
  if i>1
    % draw a white polygon obscure the parent grid - otherwise where the
  ...
  if add_vectors
    [thedata{i},thegrid{i},thehan{i}] = ...
      roms_zview(file,var,time,z,grd,vec_d,uscale,varargin{:});
  else
    [thedata{i},thegrid{i},thehan{i}] = ...
      roms_zview(file,var,time,z,grd);
  end
